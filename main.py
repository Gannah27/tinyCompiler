...
def tokenization(code_in):
    ...
    if code_in[0] == tokensLoop[0]:  # if it starts with until
        ...
        while code_in[i] != tokensLoop[1] and i< len(code_in):
            if (len(code_in)-1)%4==0 and (re.fullmatch(ID, code_in[i])) and (re.fullmatch(assignment, code_in[i+1])) and (re.fullmatch(ID, code_in[i+2]) or re.fullmatch(NUM, code_in[i+2])) and (code_in[i + 3] == tokensID[1]):
                i+= 4
